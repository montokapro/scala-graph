...
class ChoiceSpec extends AnyFunSpec {
  ...
  it("should insert uniquely and idempotently") {
    val program = db.transactor.use { xa =>
      for {
        ...
        key0 <- alg.Choice(xa).insert(Vector(foo, bar))
        key1 <- alg.Choice(xa).insert(Vector(foo, bar, baz))
        key2 <- alg.Choice(xa).insert(Vector(baz, bar, foo))
  ...
  it("should lookup values") {
    val program = db.transactor.use { xa =>
      for {
        _ <- db.Junction.setup.transact(xa)
        _ <- alg.Choice(xa).insert(Vector(foo, bar))
        key <- alg.Choice(xa).insert(Vector(foo, bar, baz))
        values <- alg.Choice(xa).lookupValues(key).compile.toList
      } yield values
    }
  ...
  it("should lookup key") {
    val program = db.transactor.use { xa =>
      for {
        ...
        key0 <- alg.Choice(xa).insert(Vector(foo, bar))
        key1 <- alg.Choice(xa).insert(Vector(foo, bar, baz))
        keys <- alg.Choice(xa).lookupKeys(bar).compile.toList
